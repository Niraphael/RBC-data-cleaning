...
#boxplot
boxplot(salary~jobcat,data=employee,main="Salary by Job catergory", 
...
ggplot(employee) + geom_point(aes(x = salbegin, y = salary))
...
my_data_frame <- recode(my_data_frame$Gender, `0` = "Male", `1` = "Female")

data1 <- read.csv("C:\\Users\\RAPHAEL PRO\\Downloads\\Eugene_Rwubaka.csv", header=TRUE, stringsAsFactors=FALSE)

# Rename the "Gender" variable using col names()
colnames(data1)[colnames(data1) == "gendeR"] <- "Gender"

data1 <- recode(data1$Gender, `0` = "Male", `1` = "Female")

data1 <-data1 %>% 
  recode(Gender_new = case_when(Gender == "0"  ~ "male",
                              Gender == "1"  ~ "Female",
                              Gender == "2" ~ "other"))
